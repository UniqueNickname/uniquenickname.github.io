window.onload = () => {
	...
	const touch = {
		start: event => {
			...
			if (isNotSuccess($item)) {
				store.defaultY = $item.offsetTop
				store.defaultX = $item.offsetLeft
				store.differentY = event.targetTouches[0].pageY - store.defaultY
				store.differentX = event.targetTouches[0].pageX - store.defaultX
				store.itemsPosition = getItemsPositions()
			}
		},
		move: event => {
			const $item = event.target
			if (isNotSuccess($item)) {
				store.lastY = event.targetTouches[0].pageY
				store.lastX = event.targetTouches[0].pageX
				const { differentY, differentX, lastY, lastX } = store
				const coordY = lastY - differentY
				const coordX = lastX - differentX
			...
			if (isNotSuccess($item)) {
				const { defaultY, defaultX, lastY, lastX } = store
				let blockMoved = false
				store.itemsPosition.forEach((size, index) => {
					if (lastY > size.top
						&& lastY < size.bottom
						&& lastX > size.left
						&& lastX < size.right) {
						const $oldItem = store.$items[index] //? блок, который находился в ячейке, в которую мы переместили новый блок
						blockMoved = changePlacesOfItems($item, $oldItem)
						blockMoved = true
					}
				})
				if (!blockMoved) {
	...
	const DnD = {
		drag: {
			...
			start: (event) => {
				...
				if ($oldItem) {
					changePlacesOfItems($item, $oldItem)
				}
			}
		},
		drop: event => {
	...
	const changePlacesOfItems = ($item, $oldItem) => {
		...
		console.log({ $newCell, $item, $oldItem, $oldCell })
	...
	const createCell = ($line, { id, text }, colNumber) => {
		...
		$item.ontouchstart = touch.start
		$item.ontouchmove = touch.move
		$item.ontouchend = touch.end
